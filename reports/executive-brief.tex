...
Docker containers & 3 & MongoDB + Langfuse + Postgres sidecar \\
...
\node[box, right=1cm of scout, fill=darkgreen!10, draw=darkgreen] (pisdk) {Pi SDK};
\node[box, right=1cm of blaze, fill=darkgreen!10, draw=darkgreen] (langfuse) {Langfuse};

\node[cloud, right=0.8cm of pisdk, yshift=0.5cm] (local) {Ollama (free)};
\node[cloud, right=0.8cm of pisdk, yshift=-0.5cm] (cloud) {Claude/Gemini};
...
\draw[arr] (pisdk) -- (local);
\draw[arr] (pisdk) -- (cloud);
...
LLM Layer & Pi SDK & Direct provider routing + parallel execution; no proxy needed \\
Observability & Langfuse & Per-trace cost tracking; Langfuse SDK integration \\
...
{\small\textcolor{gray}{The Factory --- Executive Brief --- February 9, 2026 (revised)}}
\end{center}
