...
\begin{enumerate}[itemsep=2pt]
    ...
    \item \textbf{Minimal moving parts} --- 5 services, not 12+
...
\node[box, below=1.5cm of scout, fill=darkgreen!10, draw=darkgreen] (litellm) {LiteLLM\\{\scriptsize LLM Gateway}};
\node[box, below=1.5cm of blaze, fill=darkgreen!10, draw=darkgreen] (langfuse) {Langfuse\\{\scriptsize Observability}};

% LLM Providers
\node[cloud, below left=1.5cm and -0.5cm of litellm] (ollama) {Ollama\\{\scriptsize Local / Free}};
\node[cloud, below=1.5cm of litellm] (anthropic) {Anthropic\\{\scriptsize Frontier}};
\node[cloud, below right=1.5cm and -0.5cm of litellm] (google) {Google/Groq\\{\scriptsize Mid / Fast}};
...
\draw[arr] (kev) -- (blaze);
\draw[arr] (rex) -- (mongo);
\draw[arr] (rex) -- (litellm);
\draw[arr] (scout) -- (litellm);
\draw[arr] (blaze) -- (litellm);
\draw[arr] (litellm) -- (ollama);
\draw[arr] (litellm) -- (anthropic);
\draw[arr] (litellm) -- (google);
\draw[arr] (rex) -- (cf);
...
\midrule
\textbf{Total} & \textbf{8--10 GB} & Leaves 54+ GB for agent processes \\
\bottomrule
\end{tabular}
...
\section{Pi SDK + LiteLLM --- LLM Orchestration}

\textbf{Two-layer stack:} Pi SDK decides \emph{what} to run; LiteLLM decides \emph{where} to run it.

\begin{itemize}[itemsep=3pt]
    \item \textbf{Pi SDK} handles spawning Claude Code / Codex sessions, parallel execution, context management
    \item \textbf{LiteLLM} provides unified OpenAI-compatible API across all providers, per-key budgets, fallback chains, cost logging
\end{itemize}
...
Self-hosted Langfuse provides per-trace cost tracking, prompt management, and evaluation framework. Native LiteLLM integration via a single config line: \texttt{callbacks: ["langfuse"]}. Requires a minimal Postgres sidecar (\textasciitilde512MB RAM).
...
\begin{itemize}[itemsep=3pt]
    ...
    \item LiteLLM logs every call to MongoDB $\rightarrow$ real-time spend queries
\end{itemize}
...
\begin{itemize}[itemsep=2pt]
    \item Per-task and per-agent budget caps in LiteLLM
...
\midrule
Observability & Langfuse (self-hosted) & Grafana+Prometheus, custom & Native LiteLLM integration; trace visualization \\
\midrule
...
{\small\textcolor{gray}{Document generated February 8, 2026}}\\
